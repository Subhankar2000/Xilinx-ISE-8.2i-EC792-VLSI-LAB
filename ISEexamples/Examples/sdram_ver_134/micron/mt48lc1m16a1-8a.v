...
* ----  ----------------------------  ----------  ---------------------------------------
* 0.0e  Son Huynh       208-368-3825  12/17/1998  - Fix some problem in 0.0c
...
module mt48lc1m16a1 (Dq, Addr, Ba, Clk, Cke, Cs_n, Ras_n, Cas_n, We_n, Dqm);
    ...
    wire      Debug            = 1'b1;                          // Turn on Debug messages
    ...
    // Timing Parameters for -8A component (CL = 3)
    parameter tAC  =   6;    
    parameter tOH  =   2.5;
    parameter tHZ  =   6;
...
//        $readmemh("bank1.txt", Bank1);
    ...
    always @ (posedge Sys_clk) begin
        ...
        if (Prech_enable == 1'b1) begin
            ...
            if (Cas_latency_3 == 1'b1) begin
                Command[2] = `PRECH;
                Bank_precharge[2] = Ba;
                A10_precharge[2] = 1'b1;
            end else if (Cas_latency_2 == 1'b1) begin
                Command[1] = `PRECH;
                Bank_precharge[1] = Ba;
                A10_precharge[1] = 1'b1;
            end else if (Cas_latency_1 == 1'b1) begin
                Command[0] = `PRECH;
                Bank_precharge[0] = Ba;
                A10_precharge[0] = 1'b1;
            end
    ...
    // Timing Check for -8A component (CL = 3)
    specify
        specparam
   ...
   //     $setuphold(posedge Clk,    Cke,   tCKS, tCKH);		//Commented out for functional simulation
   //     $setuphold(posedge Clk,    Cs_n,  tCMS, tCMH);		//Commented out for functional simulation
   //     $setuphold(posedge Clk,    Cas_n, tCMS, tCMH);		//Commented out for functional simulation
   //     $setuphold(posedge Clk,    Ras_n, tCMS, tCMH);		//Commented out for functional simulation
   //     $setuphold(posedge Clk,    We_n,  tCMS, tCMH);		//Commented out for functional simulation
   //     $setuphold(posedge Clk,    Addr,  tAS,  tAH);		//Commented out for functional simulation
   //     $setuphold(posedge Clk,    Ba,    tAS,  tAH);		//Commented out for functional simulation
   //     $setuphold(posedge Clk,    Dqm,   tCMS, tCMH);		//Commented out for functional simulation
   //     $setuphold(posedge Dq_chk, Dq,    tDS,  tDH);		//Commented out for functional simulation
    endspecify
