	...
	if (Reset) begin
		...
		if (PS2_Clk)	begin
			...
			if (clk_high_filter == 3'b011)
				filter_clk <= 1'b1;		  //If PS2_Clk remains high for 7 CLK cycles, filter_clk transitions high
		end
		else if (!PS2_Clk) begin
			clk_high_filter <= 3'b000;
			clk_low_filter <= clk_low_filter + 3'b001;
			if (clk_low_filter == 3'b011)
				filter_clk <= 1'b0;			//If PS2_Clk remains low for 7 CLK cycles, filter_clk transitions low
		end
	end		
end
...
always @(posedge Clk or posedge Reset)	begin
	if (Reset) begin
		...
		if ((trigger) && (DoRead) && (filter_clk)) begin   
...
always @(posedge filter_clk or posedge Reset)
